...
class WebApiRequest {
    ...
    var urlBase = "https://api.nal.usda.gov/ndb/search/?"
    var max = 25
    ...
    func sendRequest<T:Decodable>(toUrlPath urlPath: String, completion: @escaping (T)->Void) {
        ...
        if apiKey.isEmpty{
            ...
            //let url = urlBase + "max=" + max + "&api_key=" + apiKey + "q=" + encodedUrlPath;
            guard let url = URL(string: "\(urlBase)max=\(max)&api_key=\(apiKey)&q=\(encodedUrlPath!)") else {
